...
def _dashboard_plan(state: GraphState, config: RunnableConfig) -> Dict[str, DashboardPlanner]:
    ...
    additional_info = "This is the dataframes needed for plotting:" + str(all_df_metadata.get_schemas_and_samples())
    
    # print(additional_info)
    # message_plus = query.append({"type": "text", "text": additional_info})
    # print(message_plus)
    # message_updated = HumanMessage(content=message_plus)
    message_updated = [SystemMessage(additional_info)] + message


    model_with_tools = llm.bind_tools([DashboardPlanner])
    ...
    try:
        dashboard_plan = model_with_tools.invoke(message_updated)
